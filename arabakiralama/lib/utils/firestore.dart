...
//Users uid altında cart collection yarattık
//Cart a ekleye tıklanınca ilk araba exists mi kontrol ediliyor
//Eğer exists se count update +1 yapılıyor
...
class FirestoreUtil {
  ...
  static Future<List<Product>> getProducts(List<String>? ids) async {
    try {
      ...
      log("Araba getirilemedi", stackTrace: stacktrace, error: e);
    }
    return [];
  }
  ...
  static addToCart(User? user, String productId) async {
    ...
    try {
      ...
      log("Cart'a eklerken hata oluştu", stackTrace: stackTrace, error: e);
    }
  }
  ...
  static Future<List<Cart>> getCart(User? user) async {
    ...
    try {
      ...
      log("Cart getirilirken hata oluştu", stackTrace: stackTrace, error: e);
    }
    return carts;
  }
